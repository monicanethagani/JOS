...
//	extern struct Segdesc gdt[];
	...
	// default initialize all IRQs so we are at least aware that an unhandled interrupt occured
	for (int i = 0; i < 15; i++) {
...
//	gdt[i(GD_TSS0 >> 3) + thiscpu->cpu_id] = SEG16(STS_T32A, (uint32_t) (&ts),
//					sizeof(struct Taskstate) - 1, 0);
...
page_fault_handler(struct Trapframe *tf)
{
	...
	uintptr_t sb = UXSTACKTOP - PGSIZE;
	uintptr_t st = UXSTACKTOP;
	if (tf->tf_esp >= sb && tf->tf_esp < UXSTACKTOP)
	{
		st = tf->tf_esp - 4;
	}

	struct UTrapframe *u = (struct UTrapframe *)(st - sizeof(struct UTrapframe));
